...
function renderHomePage() {
  ...
  if (viewAssessmentBtn) {
    viewAssessmentBtn.addEventListener('click', async () => {
      try {
        const response = await fetch('http://localhost:5000/api/get-latest-assessment', {
          method: 'POST',
          headers: {
...
async function saveAnswer(questionNumber, answer, question) {
  return makeAPIRequest('save-answer', 'POST', {
    userId: clerk.user.id,
    email: clerk.user.primaryEmailAddress?.emailAddress,
...
if (clerk.user) {
  // Check if this is a new user (no goals set yet)
  const goalsFile = 'data/user_goals.json';
  fetch('http://localhost:5000/api/check-user-goals', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  .catch(error => {
    ...
    // Default to goal page for new users if check fails
    renderGoalPage();
  });
} else {
  // Modified sign in page with two options
  document.getElementById("app").innerHTML = `
